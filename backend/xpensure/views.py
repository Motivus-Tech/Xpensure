...
def process_approval(request_obj, approver_employee, approved=True, rejection_reason=None):
    ...
    if next_approver_id:
        try:
            ...
            # ✅ ✅ ✅ FIXED: CEO approval ALWAYS sets status to "Approved"
            if approver_employee.role == "CEO":
                request_obj.status = "Approved"
                print(f"✅ CEO approved: Status = 'Approved' (ready for payment)")
                
                # Try to find Finance Payment user (even if not next in chain)
                finance_payment_user = User.objects.filter(role="Finance Payment").first()
                if finance_payment_user:
                    # Override to assign to Finance Payment
                    request_obj.current_approver_id = finance_payment_user.employee_id
                    print(f"✅ Reassigned to Finance Payment: {finance_payment_user.employee_id}")
            else:
        ...
        request_obj.status = "Approved"  # ✅ Always "Approved" when no next approver
        request_obj.current_approver_id = None
        request_obj.final_approver = approver_employee.employee_id
        ...
        if approver_employee.role == "CEO":
            ...
            if finance_payment_user:
                request_obj.current_approver_id = finance_payment_user.employee_id
                request_obj.status = "Approved"  # This is already "Approved"
...
class FinanceMarkAsPaidView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            # 1. Check if request is assigned to Finance Payment user
            if obj.current_approver_id != request.user.employee_id:
                print(f"❌ Request not assigned to current user")
                return Response({
                    "error": "This request is not assigned to you for payment processing.",
                    "details": {
                        "assigned_to": obj.current_approver_id,
                        "your_id": request.user.employee_id
                    }
                }, status=400)
            
            # 2. Check if status is "Approved" 
            if obj.status != "Approved":
                print(f"❌ Status is not 'Approved': {obj.status}")
                return Response({
                    "error": f"Cannot mark as paid. Current status: {obj.status}. Required: 'Approved'",
                    "details": {
            ...
            # ✅ MARK AS PAID
            print(f"✅ Validations passed. Marking as paid...")
            
            obj.status = "Paid"
            obj.payment_date = timezone.now()
            obj.current_approver_id = None  # No more approvers needed
            obj.save()
            ...
            print(f"✅ Successfully marked as paid!")
